...
def clean_company_name(name):
    """Cleans company names by removing legal suffixes and irrelevant characters."""
...
def is_valid_record(record):
    """Checks if a record has at least one non-empty field."""
    return bool(
...
def process_block_batch(block_dict, deduper, batch_size=1000):
    """Processes a block in batches to manage memory."""
...
def main():
    ...
    for key, block in blocks:
        if len(block) > 1:
            # Sample up to 2 records per block
            sample_size = min(len(block), 2)
    ...
    if not training_sample:
        print("Error: No valid sample found for training.")
        logging.error("No valid sample found for training.")
        return
        
    deduper.prepare_training(training_sample)
    print("Interactive training (classify the pairs, press 'f' to finish when ready)...")
    logging.info("Interactive training (classify the pairs, press 'f' to finish when ready)...")
    console_label(deduper)  # This will use pre-existing data if present in training.json
    deduper.train(recall=0.5, index_predicates=True)
    ...
    try:
        ...
        print(f"Error while saving 'training.json': {str(e)}")
        logging.error(f"Error while saving 'training.json': {str(e)}")
    
    del training_sample
    gc.collect()
    
    # Block-by-block deduplication
    all_clusters = []
    ...
    print(f"Without duplicates: {(df['cluster_id'] == -1).sum()}")
    logging.info("Results:")
    logging.info(f"Total records: {len(df)}")
    logging.info(f"Clusters: {len(set(cluster_membership.values()))}")
    logging.info(f"Without duplicates: {(df['cluster_id'] == -1).sum()}")
